...
module.exports = (app) => {
  ...
  router.get("/", fetchUser, async (req, res) => {
    ...
    const result = await moduleModel.getAllRecords();
    if (result) {
  ...
  router.post("/", fetchUser, async (req, res) => {
    ...
    if (req.body.hasOwnProperty("icon")) {
      module_record.message = icon;
      if (!icon) {
        errors.push("icon is required");
      }
    }
    if (req.body.hasOwnProperty("order_no")) {
      module_record.message = order_no;
      if (!order_no) {
        errors.push("order_no is required");
      }
    }
    if (req.body.hasOwnProperty("status")) {
      module_record.message = status;
    ...
    if (req.body.hasOwnProperty("api_name")) {
      module_record.message = api_name;
      if (!api_name) {
  ...
  router.put("/:id", fetchUser, async (req, res) => {
    try {
      ...
      if (req.body.hasOwnProperty("order_no")) {
        module_record.order_no = order_no;
        if (!order_no) {
      ...
      if (req.body.hasOwnProperty("status")) {
        module_record.status = status;
        if (!status) {
          errors.push("status is required");
        }
      }
      if (req.body.hasOwnProperty("icon_type")) {
        module_record.icon_type = icon_type;
        if (!icon_type) {
          errors.push("icon type is required");
        }
      }
      if (req.body.hasOwnProperty("icon")) {
        module_record.icon = icon;
        if (!icon) {
          errors.push("icon name is required");
        }
      }
      if (req.body.hasOwnProperty("url")) {
        module_record.url = url;
        if (!url) {
      ...
      if (req.body.hasOwnProperty("parent_module")) {
        module_record.parent_module = parent_module;
      }
      if (errors.length !== 0) {
        return res.status(400).json({ errors: errors });
      }
      ...
      moduleModel.init(req.userinfo.tenantcode);
      const isDuplicate = await moduleModel.checkDuplicateRecord(
        req.body.name,
        req.params.id
      );
