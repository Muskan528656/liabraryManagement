...
module.exports = (app) => {
  ...
  router.post(
    ...
    [
      ...
      body("adminemail").isEmail().withMessage("Valid admin email is required"),
      body("isactive").isBoolean().withMessage("isActive must be a boolean value"),
      // Added validation for currency_symbol (Optional but validated if present)
  ...
  router.put(
    ...
    [
      ...
      body("adminemail").isEmail().withMessage("Valid admin email is required"),
      body("isactive").isBoolean().withMessage("isActive must be a boolean value"),
      // Added validation for currency_symbol here too
      body("currency_symbol").optional().isString().withMessage("Currency symbol must be a string"),
      body("currency").optional().isString().withMessage("Currency must be a string"),
    ],
    async (req, res) => {
      try {
        ...
        const userId = req.userinfo?.id || null;
        // req.body now contains currency_symbol sent from frontend
        const companyData = { ...req.body };
        const previousImagePath = existingCompany.logourl;
