...
async function getAllRecords() {
  let query = `SELECT * FROM demo.module `;
...
async function createRecord(reqBody) {
  const order_no =
    reqBody.order_no !== null ? parseInt(reqBody.order_no) : null;

  const result = await sql.query(
    `INSERT INTO public.module (name, status, api_name, icon, url, icon_type, order_no, parent_module) VALUES ($1, $2,$3,$4,$5,$6,$7,$8) RETURNING *`,
    [
      reqBody.name,
      reqBody.status,
      reqBody.api_name,
