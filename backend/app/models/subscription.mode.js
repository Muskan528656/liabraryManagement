...
async function findAll() {
    const query = `SELECT * FROM demo.subscriptions ORDER BY createddate DESC`;
...
async function create(data, userId) {
    const query = `
        ...
        (plan_name, start_date, end_date, is_active, "allowed books", createdbyid, lastmodifiedbyid, createddate, lastmodifieddate)
    ...
    const values = [
        ...
        data.allowed_books ?? data["allowed books"] ?? null,
        userId || null
    ];
...
async function updateById(id, data, userId) {
    ...
    const values = [
        ...
        data.start_date && data.start_date.trim() !== "" ? data.start_date : (current.start_date || null),
        data.end_date && data.end_date.trim() !== "" ? data.end_date : (current.end_date || null),
        data.is_active ?? current.is_active,
        data.allowed_books ?? data["allowed books"] ?? current.allowed_books ?? current["allowed books"],
        userId || null
    ];
    const result = await sql.query(query, values);
    return result.rows.length ? normalizeSubscriptionRow(result.rows[0]) : null;
