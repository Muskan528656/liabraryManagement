...
export default function Header({ open, handleDrawerOpen, socket }) {
  ...
  // Combined notifications for display
  const combinedNotifications = [...dueNotifications, ...allNotifications];
  ...
  // Handle notification click - marks as read and performs action
      ...
      >
        ...
        <div className="d-flex align-items-center gap-2">
          ...
          >
            ...
            >
              <i className="fa-solid fa-bell" style={{ fontSize: "20px" }}></i>
              {calculateTotalUnread() > 0 && (
                <span
                  className="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger"
                  style={{ fontSize: "10px", padding: "2px 5px" }}
                >
                  {calculateTotalUnread() > 99 ? "99+" : calculateTotalUnread()}
                </span>
              )}
            </Dropdown.Toggle>
