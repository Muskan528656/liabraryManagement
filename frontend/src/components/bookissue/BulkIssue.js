...
const BulkIssue = () => {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      // System default को settings से लें या 6 रखें
      setSystemMaxBooks(6);
  ...
  const loadMemberInfo = async (cardId) => {
    try {
      ...
      if (memberResp && memberResp.data) {
        ...
        // पहले member से plan_id/planId/subscription_id/subscriptionId निकालें
        let planId = null;

        // विभिन्न possible field names से plan id निकालें
        if (member.plan_id) {
        ...
        if (planId && subscriptions.length > 0) {
          // subscription list में plan ढूंढें
          subscription = subscriptions.find(sub => {
            // विभिन्न possible id fields check करें
            return (
        ...
        // System max books अब subscription/plan के allowed books होंगे
        const systemMax = subscriptionBooks > 0 ? subscriptionBooks : 6;
        setSystemMaxBooks(systemMax);

        // Total allowed books की गणना
          ...
          {memberSubscription ? (
            <>
              ...
              <div className="ps-2">
                ...
                <div>Allowed: {systemMaxBooks} books</div>
