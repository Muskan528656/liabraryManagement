...
// //     // General books (जिनकी कोई age restriction नहीं) - हमेशा show होनी चाहिए
// //     const isGeneralBook = (minAge === 0 && maxAge === 0) || 
// //                           (minAge === 0 && maxAge === 999) ||
// //                           (book.min_age === null && book.max_age === null);

// //     // Specific age range books - check if member's age falls within book's range
// //     const isAgeAppropriate = memberAge >= minAge && memberAge <= maxAge;

// //     return isGeneralBook || isAgeAppropriate;
// //   });
...
//   if (!selectedCard || memberAge === null) {
//     // अगर member select नहीं है, तो सभी books show करें
//     setFilteredBooksByAge(books);
//     return;
//   }

//   // Member की age के according books filter करें
//   const filtered = books.filter(book => {
//     const minAge = parseInt(book.min_age) || 0;
//     const maxAge = parseInt(book.max_age) || 999;

//     console.log(`Book: ${book.title}, minAge: ${minAge}, maxAge: ${maxAge}, memberAge: ${memberAge}`);

//     // Case 1: General books (All ages) - जिनकी min_age = 0 और max_age = 0
//     // या जिनकी कोई age restriction नहीं है
//     const isGeneralBook = (minAge === 0 && maxAge === 0) || 
//                           (minAge === 0 && maxAge === 999) ||
//                           (book.min_age === null && book.max_age === null);

//     // Case 2: Age-specific books
//     // Book की min_age और max_age के बीच member की age होनी चाहिए
//     // लेकिन आपकी requirement के according:
//     // 1. अगर book की min_age = 4 और max_age = 75 है
//     // 2. और member की age = 11 है
//     // तो यह book नहीं दिखनी चाहिए क्योंकि min_age 4 है (member से कम)

//     // सही logic: book की min_age ≤ memberAge ≤ book की max_age
//     // लेकिन आपको चाहिए: book की min_age लगभग memberAge के equal हो
//     const isAgeAppropriate = memberAge >= minAge && memberAge <= maxAge;

//     // DEBUG: हर book के लिए check करें
//     if (isGeneralBook || isAgeAppropriate) {
//       console.log(`✓ INCLUDED: ${book.title} (${minAge}-${maxAge}) for age ${memberAge}`);
//     } else {
//       console.log(`✗ EXCLUDED: ${book.title} (${minAge}-${maxAge}) for age ${memberAge}`);
//     }
...
//   setFilteredBooksByAge(filtered);
//   console.log("Age filtering result:", {
//     memberAge,
//     totalBooks: books.length,
//     filteredBooks: filtered.length,
//     books: filtered.map(b => `${b.title} (${b.min_age}-${b.max_age})`)
//   });
...
//   // पहले ही filter हो चुकी books
//   const booksToShow = filteredBooksByAge;

//   console.log("getBookOptions - Current state:", {
//     memberAge,
//     filteredBooksCount: booksToShow.length,
//     allBooksCount: books.length,
//     sampleBooks: booksToShow.slice(0, 5).map(b => ({
//       title: b.title,
//       ageRange: `${b.min_age || 0}-${b.max_age || 999}`
//     }))
//   });

//   // सिर्फ एक group में सभी books दिखाएं (no grouping needed)
//   if (booksToShow.length === 0) {
//     return [{
//       label: `No books available for age ${memberAge} years`,
//       options: []
//     }];
//   }

//   return [{
//     label: `Books suitable for age ${memberAge} years (${booksToShow.length})`,
//     options: booksToShow.map((book) => {
//       const minAge = parseInt(book.min_age) || 0;
//       const maxAge = parseInt(book.max_age) || 999;
