...
const BookPopularityReport = () => {
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      alert("Excel export failed. Please ensure the backend endpoint exists.");
    } finally {
      setExporting(false);
    }
  };
  ...
  const columns = [
    ...
    //     align: "center",
    //     render: (val) => (
    //         <Badge bg={val === 'High' ? 'success' : val === 'Medium' ? 'warning' : 'secondary'}>
    //             {val}
    //         </Badge>
    //     )
    // },
  ...
  return (
    <div className="container-fluid bg-light min-vh-100 pb-5">
      <div className="card shadow-sm border-0 mt-4 mx-2">
        {/* Header */}
