...
const BulkPurchasePage = () => {
        ...
        try {
            ...
            if (googleResponse.ok) {
                ...
                if (data.items && data.items.length > 0) {
                    ...
                    // If no results, check with title search
                    ...
                    if (titleResponse.ok) {
                        ...
                        if (titleData.items && titleData.items.length > 0) {
                            ...
                            bookData.title = volumeInfo.title || isbn;
    ...
    const processBarcodeData = async (barcode) => {
        ...
        try {
            ...
            // If not valid ISBN but has numbers and letters, try to extract ISBN
            if (/\d/.test(barcode) && /[A-Za-z]/.test(barcode)) {
                const possibleIsbns = barcode.match(/[\dX]{10,13}/g);
                if (possibleIsbns) {
                    for (const possibleIsbn of possibleIsbns) {
        ...
        try {
            ...
            if (processedData) {
                ...
                setBookFormData(prev => ({
                    ...prev,
                    isbn: barcode,
                    title: barcode.length > 50 ? barcode.substring(0, 50) + "..." : barcode
                }));
                toast.info("Could not auto-detect book details. Please fill manually.");
            }
        } catch (error) {
    ...
    const handleAddBook = async () => {
        ...
        try {
            ...
            if (response && response.data) {
                ...
                // Try different ways to get book ID
                newBookId = bookData.id || bookData._id ||
